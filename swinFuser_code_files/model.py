...
from swin_transfuser_ptt import SwinPTTBackbone
...
class LidarCenterNet(nn.Module):
    ...
    def __init__(self, config, device, backbone, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        ...
        elif(backbone == 'swin_ptt'):  # NEW OPTION
            self._model = SwinPTTBackbone(config, image_architecture, lidar_architecture, use_velocity=use_velocity).to(self.device)
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, save_path=None, expert_waypoints=None,
                    stuck_detector=0, forced_move=False, num_points=None, rgb_back=None, debug=True):
        ...
        if (self.backbone == 'transFuser'):
            ...
            raise("The chosen vision backbone does not exist. The options are: transFuser, late_fusion, geometric_fusion, latentTF, swin_ptt")
