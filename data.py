...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config, shared_dict=None):
        print(f"Root directories received: {root}")
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            sub_root = Path(sub_root).resolve()
            print(f"\nProcessing sub_root: {sub_root} (real path: {sub_root})")
            
            if sub_root in real_paths:
                print(f"Skipping already processed: {sub_root}")
                continue
            real_paths.add(sub_root)
            
            routes = [folder for folder in os.listdir(sub_root) if os.path.isdir(sub_root / folder)]
            print(f"Found routes: {routes}")

            for route in routes:
                route_dir = (sub_root / route).resolve()
                print(f"\nProcessing route: {route_dir}")
                
                if route_dir in real_paths:
                    print(f"Skipping cyclic route: {route_dir}")
                    continue
                real_paths.add(route_dir)
                
                lidar_dir = route_dir / "lidar"
                print(f"Lidar dir exists? {lidar_dir.exists()}")
                
                if not lidar_dir.exists():
                    print(f"Skipping {route_dir}: 'lidar' missing")
                    continue
                
                num_seq = len(os.listdir(lidar_dir))
                print(f"Lidar frames: {num_seq}")
    ...
    def __getitem__(self, index):
        ...
        if (backbone == 'geometric_fusion'):
            curr_bev_points, curr_cam_points = lidar_bev_cam_correspondences(deepcopy(lidars_raw), debug=True)
...
def lidar_bev_cam_correspondences(world, lidar_vis=None, image_vis=None, step=None, debug=True):
    """
    ...
    if (debug == True):
        ...
        plt.ion()
        plt.imshow(vis_bev)
        plt.savefig(r'D:/Lab6/transfuser/team_code_transfuser/save_folder/Visualizations/2/bev_lidar_{}.png'.format(step), bbox_inches='tight')
        #plt.savefig(r'D:/Lab6/transfuser/logs/Visualizations/2/bev_lidar_{}.png'.format(step), bbox_inches='tight')

        plt.close()
        plt.imshow(vis_original_image)
        plt.savefig(r'D:/Lab6/transfuser/team_code_transfuser/save_folder/Visualizations/2/image_with_lidar_{}.png'.format(step), bbox_inches='tight')
        #plt.savefig(r'D:/Lab6/transfuser/logs/Visualizations/2/image_with_lidar_{}.png'.format(step), bbox_inches='tight')
