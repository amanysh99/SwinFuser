...
class GlobalConfig:
    ...
    #LAV Multi-agent learning configuration (LAV-inspired)
    use_multi_agent = True  # Enable multi-agent learning
    max_agents = 10  # Maximum number of agents to track
    agent_vicinity_radius = 50.0  # Radius in meters to consider nearby agents
    multi_agent_loss_weight = 0.5  # Weight for multi-agent losses
    
    # Multi-agent feature dimensions
    agent_feature_dim = 512  # Dimension of agent features
    agent_encoder_layers = 4  # Number of transformer layers for agent encoding
    agent_encoder_heads = 8  # Number of attention heads
    
    # Interaction modeling
    use_spatial_attention = True  # Use spatial relationships between agents
    collision_threshold = 2.0  # Distance threshold for collision detection
    interaction_radius = 30.0  # Radius for agent interactions
    
    # Training strategy
    agent_dropout = 0.1  # Dropout rate for agent features
    curriculum_learning = True  # Start with fewer agents, gradually increase
    initial_max_agents = 3  # Initial number of agents for curriculum learning
    curriculum_epoch_step = 5  # Increase agent count every N epochs
    
    # Loss weights for multi-agent objectives
    ma_trajectory_weight = 1.0
    ma_confidence_weight = 0.1
    ma_interaction_weight = 0.2
    
    # Data augmentation for multi-agent
    agent_augment = True  # Apply augmentation to agent features
    agent_position_noise = 0.5  # Noise added to agent positions
    agent_feature_dropout = 0.1  # Random dropout of agent features
    ...
    def __init__(self, root_dir='', setting='all', **kwargs):
        ...
        self.root_dir = "D:/Lab6/transfuser/data"
        now = datetime.datetime.now()

        self.log_dir = os.path.join("D:/Lab6/transfuser/logs", now.strftime("%Y-%m-%d_%H-%M-%S"))#win
...
class GlobalConfig:
    """ base architecture configurations """
    no_shift = False  # Enable shifted windows by default
    mlp_ratio = 4.0  # Standard for Swin Transformers
    ...
    lidar_resolution_width  = 256 # Width of the LiDAR grid that the point cloud is voxelized into.
    lidar_resolution_height = 256 # Height of the LiDAR grid that the point cloud is voxelized into.
    lidar_channels = 32 # A common value, you might need to adjust this based on your desired LiDAR setup
    lidar_points_per_second = 100000 # A common value; adjust as needed for your specific setup/model
    lidar_range = 50.0 # Maximum detection range in meters. Adjust as needed.
    lidar_rotation_frequency = 10.0 # Common value. Adjust as needed.

    pixels_per_meter = 8.0 # How many pixels make up 1 meter. 1 / pixels_per_meter = size of pixel in meters
    lidar_pos = [1.3,0.0,2.5] # x, y, z mounting position of the LiDAR
    lidar_rot = [0.0, 0.0, -90.0] # Roll Pitch Yaw of LiDAR in degree
    pretrained_bc_model_path = "D:/Lab6/transfuser/model_ckpt/transfuser/model_seed1_39.pth"

    camera_pos = [1.3, 0.0, 2.3] #x, y, z mounting position of the camera
    camera_width = 960 # Camera width in pixel
    camera_height = 480 # Camera height in pixel
    use_lidar = True
    lidar_x_range = 50.0  # Example value: +/- 25 meters in X direction
    lidar_y_range = 50.0
    camera_fov = 120 #Camera FOV in degree
    camera_rot_0 = [0.0, 0.0, 0.0] # Roll Pitch Yaw of camera 0 in degree
    camera_rot_1 = [0.0, 0.0, -60.0] # Roll Pitch Yaw of camera 1 in degree
    ...
    rl_clip_range = 0.2
    rl_ent_coef = 0.01 
    total_timesteps =10_000 # suppose to be 1_000_000 
    num_class = 7
    classes = {
        0: [0, 0, 0],  # unlabeled
        1: [0, 0, 255],  # vehicle
    ...
    def __init__(self, root_dir='', setting='all', **kwargs):
        self.max_speed_control = 30.0 
        self.target_speed = 20.0 
        self.save_freq = 10000
        now = datetime.datetime.now()
        self.log_dir = os.path.join("D:\\Lab6\\transfuser\\logs", now.strftime("%Y-%m-%d_%H-%M-%S"))#win
        #self.log_dir = os.path.join("/mnt/d/Lab6/transfuser/logs", now.strftime("%Y-%m-%d_%H-%M-%S"))#linux

        self.root_dir = "D:\\Lab6\\transfuser\\team_code_transfuser\\data"
        if (setting == 'all'): # All towns used for training no validation data
            self.train_towns = os.listdir(self.root_dir)
            self.val_towns = [self.train_towns[0]]
            self.train_data, self.val_data = [], []
